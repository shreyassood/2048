            ...
            for (int yy: traversalsY) {
                ...
                if (tile != null) {
                    ...
                    if (next != null && next.getValue() == tile.getValue() && next.getMergedFrom() == null) {
                        ...
                        mView.sound();
                        if(merged.getValue()>maxmerged){
                        	maxmerged = merged.getValue();
                        	Log.d("A", String.valueOf(maxmerged));
                        }
                        if (merged.getValue() == 1024) {
                        	mView.almost();
                        }
                        if (merged.getValue() == 2048) {
                        	if (!this.displayedwon){
                        	mView.winner();
                        	Toast toast = Toast.makeText(mContext, "You won, Congratulations! Now let's see how far you can go.", Toast.LENGTH_LONG);
                        	toast.show();
                        	displayedwon = true;
                        	}
                        }
                        if (merged.getValue() == 4096) {
                        	mView.success();
                        }
                    } else {
